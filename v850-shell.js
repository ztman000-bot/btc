/* BTC Hedge Assistant v8.20.3 - Modern Mobile Shell compatibility */
(()=>{
'use strict';
const FALLBACK_VERSION='8.20.3';
const NAV=[['home','홈'],['scanner','스캐너'],['position','포지션'],['daily','일일 시황'],['more','더보기']];
const MORE=[['trade','실전 매매표','현재 가격구간 행동 로드맵'],['settings','전략 설정','지표·안전거리·거래량 기준'],['alerts','알림','가격·전략 알림 관리'],['backtest','백테스트','전략 성과와 MDD 검증'],['log','실행 기록','판단 실행 및 학습 기록'],['chart','차트','지표 차트와 가격 레벨 확인']];
const $=id=>document.getElementById(id);
const TOOL_IDS=new Set(['scanner','position','dailyBriefPane','trade','settings','alerts','backtest','log','v851ChartPane','v850MorePane']);
...
function isToolPane(el){if(!el||!el.id)return false;return TOOL_IDS.has(el.id)}
...
function adoptModernHome(){const main=document.querySelector('main');if(!main||!$('v850HomePane'))return;[...main.children].forEach(moveHomeNode)}
function markToolPages(){TOOL_IDS.forEach(id=>{const el=$(id);if(el&&el.id!=='v850MorePane')el.classList.add('v853Page')});const chart=$('chart')?.closest('.card');if(chart){if(!chart.id)chart.id='v851ChartPane';chart.classList.add('v853Page')}}
...
function openTool(id){if(id==='chart'){const c=$('chart')?.closest('.card');if(c){if(!c.id)c.id='v851ChartPane';return activatePane(c,'more')}}return direct(id,'more')||showMore()}
...
function watchNewHomeCards(){const main=document.querySelector('main');if(!main)return;new MutationObserver(muts=>{for(const m of muts)for(const n of m.addedNodes){if(n.nodeType===1&&!isToolPane(n))moveHomeNode(n)}}).observe(main,{childList:true})}
function init(){injectStyle();document.body.classList.remove('v851','v852');document.body.classList.add('v853');patchVersion();if(!ensurePages())return;markToolPages();adoptModernHome();buildFooter();watchNewHomeCards();const h=(location.hash||'#home').slice(1);route(NAV.some(x=>x[0]===h)?h:'home');setTimeout(()=>{patchVersion();adoptModernHome();try{window.BTCResearchShadow?.heal?.();window.BTCResearchHealth?.render?.();window.BTCRecoveryV2UI?.render?.()}catch(e){}},1200)}
...
window.BTCV853={version:FALLBACK_VERSION,route,activatePane,showHome,showDaily,adoptModernHome};
})();