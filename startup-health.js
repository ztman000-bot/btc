/* BTC Hedge Assistant v8.22.0 - persistent startup/system health registry */
(function(){'use strict';
...
function host(){return window.BTCUIHomeSlots?.get?.('health')||window.BTCV853?.slot?.('health')||document.getElementById('v850HealthSlot')||document.getElementById('v850HomePane')||document.querySelector('main')}
function ensure(){let el=document.getElementById(ID);if(!el){el=document.createElement('div');el.id=ID;el.className='card';el.style.cssText='border:1px solid #35517a;background:#0d1722;margin:0 0 10px;max-width:1180px'}const p=host();if(p&&el.parentElement!==p)p.appendChild(el);return el}
...
window.BTCStartupHealth={version:'2.2',status:()=>last||state(),render};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
})();