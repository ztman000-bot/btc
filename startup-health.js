/* BTC Hedge Assistant v8.20.0 - persistent startup/system health registry */
(function(){'use strict';
  ...
  function state(){var c=checks(),miss=c.filter(function(x){return x.critical&&!x.ok}),soft=c.filter(function(x){return !x.critical&&!x.ok}),guard=window.BTCSafetyCore?.current?.()||null;return{version:window.BTC_APP_VERSION||'--',ready:miss.length===0,criticalMissing:miss.map(function(x){return x.label}),optionalMissing:soft.map(function(x){return x.label}),guard:guard,checks:c,at:Date.now()}}
