/* BTC Hedge Assistant v8.4.0 - Daily Market Brief */
(()=>{
'use strict';
const DATA_URL='./data/daily/brief.json';
const SNAP_KEY='v840_daily_brief_snapshots';
...
function ensureUI(){
 ...
 const btn=document.createElement('button');btn.id='dailyBriefTabBtn';btn.textContent='일일 시황';btn.className='tabPad';host.appendChild(btn);
 const pane=document.createElement('section');pane.id='dailyBriefPane';pane.className='tabPane';pane.style.display='none';
 pane.innerHTML=`<div class="card scannerHero"><div class="row"><div><div class="mid">V8.4 일일 시황</div><div class="small">글로벌 리포트 + 우리 시스템 기준 종합</div></div><button id="dailyBriefRefresh">새로고침</button></div><div id="dailyBriefBody" style="margin-top:10px"><div class="small">시황 생성 중...</div></div></div>`;
...
async function refresh(){
 ...
 const lines=(ext.globalConsensus||[]).map(x=>`<div class="condItem">• ${esc(x)}</div>`).join('')||'<div class="condItem">외부 컨센서스 없음</div>';
 const risks=(ext.risks||[]).map(x=>`<div class="condItem condNO">• ${esc(x)}</div>`).join('')||'<div class="condItem">특이 리스크 없음</div>';
 const events=(ext.events||[]).map(x=>`<div class="condItem">${esc(x)}</div>`).join('')||'<div class="condItem">예정 이벤트 없음</div>';
 ...
 body.innerHTML=`
 <div class="card nextActionCard"><div class="row"><div><div class="nextTitle">오늘의 한줄 시황</div><div class="nextMain">${esc(v)}</div></div><div class="scoreRing">${Math.round(finalScore)}</div></div><div class="small" style="margin-top:8px">${esc(ext.headline||'')} </div><div class="stack" style="margin-top:8px"><span class="badge ${freshClass}">${freshLabel}</span><span class="badge">외부 ${Math.round(extScore)}/100</span><span class="badge">우리 기준 ${Math.round(internalScore)}/100</span><span class="badge">종합 ${Math.round(finalScore)}/100</span></div></div>
 <div class="grid2"><div class="card"><b>글로벌 리포트 공통분모</b><div class="condGrid">${lines}</div></div><div class="card"><b>우리 시스템 판정</b><div class="levelGrid"><div class="levelBox"><div class="small">종합 신뢰도</div><div class="mid">${intl.confidence==null?'--':intl.confidence+'%'}</div></div><div class="levelBox"><div class="small">청산가 안전거리</div><div class="mid">${intl.safety==null?'--':intl.safety+'%'}</div></div><div class="levelBox"><div class="small">추세점수</div><div class="mid">${intl.trend==null?'--':intl.trend}</div></div><div class="levelBox"><div class="small">진입점수</div><div class="mid">${intl.entry==null?'--':intl.entry}</div></div></div><div class="small" style="margin-top:8px">${esc(learnTxt)}</div></div></div>
 <div class="card"><b>오늘의 3개 시나리오</b><div class="stressGrid"><div class="stressCard"><div class="small">상승</div><div class="mid good">${sc.up}%</div><div class="small">추세 유지 + 핵심 저항 돌파 확인</div></div><div class="stressCard"><div class="small">횡보</div><div class="mid warn">${sc.flat}%</div><div class="small">이벤트 전 대기·방향성 소진</div></div><div class="stressCard"><div class="small">하락</div><div class="mid bad">${sc.down}%</div><div class="small">금리·유가·위험회피 충격 확대</div></div></div></div>
 <div class="grid2"><div class="card"><b>핵심 리스크</b><div class="condGrid">${risks}</div></div><div class="card"><b>주요 일정</b><div class="condGrid">${events}</div></div></div>
 <div class="card"><div class="row"><b>출처/기준</b><span class="small">생성 ${ext.generatedAt?new Date(ext.generatedAt).toLocaleString('ko-KR'):'--'}</span></div><div class="small" style="line-height:1.7;margin-top:7px">${(ext.sources||[]).map(sourceHtml).join(' · ')||'외부 출처 없음'}<br>종합점수 = 외부 컨센서스 45% + 우리 시스템 55%. 외부자료가 36시간 이상 오래되면 외부점수는 중립 50으로 자동 보정합니다. 이 화면은 투자판단 보조이며 주문을 자동 실행하지 않습니다.</div></div>`;
