...
const V='1.0.0',KEY='btc:external-strategy-challenger:v1',MAX=600;
...
function evaluate(){
  ...
  const base=ex.base;
  const candidates=ex.candidates.map(c=>({
    id:`REDUCE_SHORT_${Number(c.qty||0).toFixed(3)}`,
    qty:Number(c.qty||0),score:scoreCandidate(c,base),
    worstMargin:c.worstMargin,maxGross:c.maxGross,maxNet:c.maxNet,minDist:c.minDist,realized:c.realized
  ...
  window.__BTC_EXTERNAL_CHALLENGER_LAST=rec;
  document.dispatchEvent(new CustomEvent('btc-external-challenger',{detail:rec}));
  return rec;
}
...
window.BTCExternalStrategyChallenger={version:V,evaluate,status,history:load};
