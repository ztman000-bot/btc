...
const V='1.0';let legacyDecide=null,legacyOutcome=null,last=null;
...
function patchDecisionUI(reason,validation){
  try{
    if(typeof currentDecision!=='undefined'&&currentDecision){currentDecision.exec=false;currentDecision.execState='금지';currentDecision.reason=(currentDecision.reason?currentDecision.reason+' · ':'')+reason}
  }catch(e){}
...
function enforceDecision(){
  ...
  const bootstrapOk=!window.BTC_BOOTSTRAP_ERROR,healthOk=window.BTC_DECISION_ALLOWED!==false,coreOk=!!core,allowed=bootstrapOk&&healthOk&&coreOk&&validation?.allowed!==false;
  let reason='';
  if(!bootstrapOk)reason='Bootstrap 오류';else if(!healthOk)reason='Runtime Health 미통과';else if(!coreOk)reason='Central Safety Core 없음';else if(validation&&!validation.allowed)reason='Central Operating Guard 미충족';
  if(!allowed)patchDecisionUI(reason||'Safety Gate 미통과',validation);
  last={allowed,reason,validation,at:Date.now()};return last;
}
function wrapLegacyDecision(){
  if(window.__BTC_LEGACY_DECIDE_WRAPPED)return true;
  if(typeof window.decide!=='function')return false;
  legacyDecide=window.decide;
  window.decide=function(){const r=legacyDecide.apply(this,arguments);enforceDecision();return r};
  window.__BTC_LEGACY_DECIDE_WRAPPED=true;return true;
}
function nearestClose(rows,targetMs,toleranceMs){
  const norm=typeof normalizeScannerRows==='function'?normalizeScannerRows(rows):Array.isArray(rows)?rows:[];if(!norm.length)return null;let best=null,diff=Infinity;for(const x of norm){const d=Math.abs(Number(x[0])-targetMs);if(d<diff){best=x;diff=d}}return!best||diff>toleranceMs?null:Number(best[4])
}
function outcomePct(entry,exit){return Number.isFinite(entry)&&Number.isFinite(exit)&&entry>0?(exit/entry-1)*100:null}
function fixedOutcome(r){
  const arr=typeof v80LoadPerf==='function'?v80LoadPerf(r):[],h1=r?.h1?.rows||r?.raw?.h1||r?.h1,h4=r?.h4?.rows||r?.raw?.h4||r?.h4;let changed=false;
  for(const x of arr){if(!x?.t||!x?.price)continue;if(x.h1==null){const v=outcomePct(Number(x.price),nearestClose(h1,x.t+3600000,4200000));if(v!=null){x.h1=v;changed=true}}if(x.h4==null){const v=outcomePct(Number(x.price),nearestClose(h4,x.t+14400000,9000000));if(v!=null){x.h4=v;changed=true}}if(x.h24==null){const v=outcomePct(Number(x.price),nearestClose(h1,x.t+86400000,5400000));if(v!=null){x.h24=v;changed=true}}}
  if(changed&&typeof v80SavePerf==='function')v80SavePerf(r,arr);return arr;
}
function patchScannerOutcome(){
  if(window.__BTC_SCANNER_OUTCOME_PATCHED)return true;
  if(typeof window.v80UpdateOutcomes!=='function')return false;
  legacyOutcome=window.v80UpdateOutcomes;window.v80UpdateOutcomes=fixedOutcome;try{v80UpdateOutcomes=fixedOutcome}catch(e){}window.__BTC_SCANNER_OUTCOME_PATCHED=true;return true;
}
